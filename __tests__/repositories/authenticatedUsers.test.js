...
it('Geting an user whose refresh token was added previously should return the user', async () => {
  // Arrange
  const expectedAuthenticationData = { accessToken: 'AC1', refreshToken: 'RFT1', renovationTimestamp: '2020-01-01T00:50:00.000Z' }
...
it('Adding an user whose refresh token is different than a string should throw PersistenceError error', async () => {
  ...
  // Act - Assert
  expect(() => authenticatedUsers.add(undefined, expectedAuthenticationData)).toThrow(PersistenceError)
  expect(() => authenticatedUsers.add(undefined, expectedAuthenticationData)).toThrow('Unable to persist an user with refresh token different than a string. Instead it was [undefined]')
})
